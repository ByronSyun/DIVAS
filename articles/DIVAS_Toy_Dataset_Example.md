...
installed. See the main `README.md` for installation instructions. This
vignette follows the current 1.x `CVXR` line (tested with `CVXR`
1.0-15).
...
matrices were generated as a benchmark scenario where the default
preprocessing reproduces the expected joint/partially shared component
pattern. In contrast, for many real-world omics analyses (e.g., COVID
case study), `rowCent = TRUE` is often preferred to remove feature-wise
baselines and focus on between-sample variation.
