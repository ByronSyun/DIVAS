# DIVAS User Guide: Analysing the Toy Dataset
...
especially regarding the required `CVXR` 1.x line (tested with `CVXR`
1.0-15).
...
Now, `datablock_list` is ready for DIVAS analysis. The required DIVAS
input is a named list of aligned feature-by-sample matrices; each matrix
should have features in rows, samples in columns, and the same sample
order across blocks. In application studies, data blocks may be
molecular modalities or other matched biological views, such as
cell-type-specific pseudo-bulk expression matrices.
...
The rank decomposition plot is derived from the fitted DIVAS
decomposition, not from the loading extraction commands. It shows:
...
This plot shows analogous diagnostics in the feature/loading space.
Users can print the returned plots directly or further modify them with
...
implementation, please visit the GitHub repository (DIVAS-v1 branch for
now) (2). The GNP dataset used in Example 2 is from the human
granulopoiesis atlas project (3,4).
